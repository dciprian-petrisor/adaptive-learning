...
secrethub run -- conda run --no-capture-output -n adaptive_learning coverage run --source='.' manage.py test --noinput # run the tests
test_ecode=$?                                                                                         # save last command exit code
secrethub run -- conda run --no-capture-output -n adaptive_learning coverage xml                                       # output xml
